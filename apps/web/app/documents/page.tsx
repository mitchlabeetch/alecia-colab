    ...
    <AppShell>
      <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">{t("nav.documents")}</h1>
        ...
        {/* Documents Grid */}
        {isLoading && isConvexAvailable ? (
          <DocumentListSkeleton />
        ) : filteredDocuments.length === 0 ? (
          <EmptyState
            icon={FileText}
            title={searchQuery ? t("search.noResults") : t("nav.noRecentDocuments")}
            description={searchQuery ? "Essayez une autre recherche" : "Créez votre premier document pour commencer"}
            action={{
